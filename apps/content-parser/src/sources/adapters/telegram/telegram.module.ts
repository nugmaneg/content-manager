...
export class TelegramModule implements OnApplicationShutdown {
  ...
  async onApplicationShutdown() {
    ...
    this.logger.log(
      `Shutting down ${this.clients.size} Telegram session(s)...`,
    );

    const disconnectPromises = Array.from(this.clients.entries()).map(
      async ([sessionName, client]) => {
        try {
          if (client && client.connected) {
            await client.disconnect();
            this.logger.log(
              `Telegram session "${sessionName}" disconnected successfully`,
            );
          } else {
            this.logger.debug(
              `Telegram session "${sessionName}" was not connected`,
            );
          }
        } catch (error) {
          this.logger.error(
            `Error disconnecting Telegram session "${sessionName}":`,
            error,
          );
        }
      },
