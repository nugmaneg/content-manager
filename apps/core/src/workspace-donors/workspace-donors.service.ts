...
export class WorkspaceDonorsService {
  ...
  async addDonor(
    user: UserFromToken,
    workspaceId: string,
    dto: AddWorkspaceDonorDto,
  ): Promise<WorkspaceDonorResponse> {
    await this.checkWorkspaceAccess(user, workspaceId);

    this.logger.log(
      `Adding donor source=${dto.sourceId} to workspace=${workspaceId} by user=${user.id}`,
    );
    return this.dbClient.addWorkspaceDonor(
      workspaceId,
      dto.sourceId,
      dto.settings,
    );
  ...
  async removeDonor(
    ...
    this.logger.log(
      `Removing donor source=${sourceId} from workspace=${workspaceId} by user=${user.id}`,
    );
  ...
  async updateDonor(
    ...
    workspaceId: string,
    sourceId: string,
    dto: UpdateWorkspaceDonorDto,
  ): Promise<WorkspaceDonorResponse> {
    await this.checkWorkspaceAccess(user, workspaceId);

    this.logger.log(
      `Updating donor source=${sourceId} in workspace=${workspaceId} by user=${user.id}`,
    );
  ...
  private async checkWorkspaceViewAccess(
    user: UserFromToken,
    workspaceId: string,
  ): Promise<void> {
    const workspace = await this.dbClient.getWorkspace(workspaceId);
