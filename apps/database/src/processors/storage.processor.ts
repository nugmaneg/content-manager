...
export class StorageProcessor extends WorkerHost {
    ...
    try {
      switch (job.name) {
        ...
        case JOBS_DATABASE.updateAiAnalysis:
          return await this.handleUpdateAiAnalysis(
            job.data as UpdateAiAnalysisPayload,
          );
  ...
  private async handleSaveContent(payload: SaveContentPayload) {
    this.logger.log(
      `Saving content from ${payload.sourceType}:${payload.sourceExternalId}`,
    );
